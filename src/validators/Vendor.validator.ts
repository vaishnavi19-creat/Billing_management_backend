...
export class VendorValidator {
    ...
    static validateGetAllVendor() {
        console.log('Validating validateGetAllVendor request...');
        return [
            query('page', 'Page must be a number').optional().isInt({ min: 1 }).withMessage('Page must be a positive integer'),
            query('limit', 'Limit must be a number').optional().isInt({ min: 1 }).withMessage('Limit must be a positive integer'),
        ];
    }
